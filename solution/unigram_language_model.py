...
import math
from typing import Collection, Counter, Dict
...
class UnigramLanguageModel:
    ...
    def __init__(self, words_probabilities: Dict[str, float], unseen_word_probability: float):
        """
    ...
    def calculate_perplexity(self, on_set: Collection[str]) -> float:
        ...
        logarithmic_sum = sum(math.log2(self.get_word_probability(word)) for word in on_set)
        return math.pow(2, (-1 / len(on_set)) * logarithmic_sum)  # as power of 2

    @classmethod
    def from_words_counter(cls, words_counter: Counter[str], smoothing_factor: float = 0):
        """
        ...
        for word, word_cnt in (+words_counter).items():  # only the words with positive frequency
            word_probabilities[word] = cls.calculate_mle_probability(word_cnt, word_space_size, smoothing_factor)
        # probability of unseen word
        unseen_word_probability = cls.calculate_mle_probability(0, word_space_size, smoothing_factor)

        return cls(word_probabilities, unseen_word_probability)

    @staticmethod
    def calculate_mle_probability(word_cnt: int, word_space_size: int, smoothing_factor: float) -> float:
        """
        ...
        """
        return ((word_cnt + smoothing_factor) /
                (word_space_size + smoothing_factor * LANGUAGE_VOCABULARY_SIZE))
    ...
    def sanity_test(self) -> bool:
        ...
        probabilities_sum = unseen_words_count * self._unseen_word_probability + sum(self._words_probabilities.values())
